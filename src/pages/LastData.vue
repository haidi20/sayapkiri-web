<template>
    <main-layout>
        <div class="grid md:mt-16 md:ml-6 ">
            <div class=" w-auto bg-white shadow-md rounded py-8 overflow-scroll ">
                <p class="mb-5 text-3xl pl-2 ">Last Data</p>
                <table class="shadow-lg bg-white w-full ">
                    <tr>
                        <th class=" label-column " >Location</th>
                        <th class=" label-column " >Account</th>
                        <th class=" label-column " >Profit</th>
                        <th class=" label-column " >Equity</th>
                        <th class=" label-column " >Floating</th>
                        <th class=" label-column hidden-mobile " >Account Name</th>
                        <th class=" label-column hidden-mobile " >ea enable</th>                        
                        <th class=" label-column hidden-mobile " >Trade</th>
                        <th class=" label-column hidden-mobile " >Broker Time</th>
                        <th class=" label-column hidden-mobile " >Created Date</th>
                        <th class=" label-column hidden-mobile " >Float Max</th>
                        <th class=" label-column hidden-mobile " >Float Trade</th>
                        <th class=" label-column hidden-mobile " >Float Date</th>
                        <th class=" label-column hidden-mobile " >Day</th>
                        <th class=" label-column hidden-mobile " >Row num</th>
                        <th class=" label-column hidden-mobile " >DD</th>
                        <th class=" label-column hidden-mobile " >Balance</th>
                        <th class=" label-column hidden-mobile " >Pnlday</th>
                    </tr>
                    <tr v-if="loading">
                        <td  class=" text-center " colspan="20">Loading</td>
                    </tr>
                    <tr v-for="(item, index) in table"  :key="index">
                        <td class=" data text-center ">
                            {{item.location}} 
                            <img 
                                class="w-5"
                                style="display:inline"
                                v-if="item.ea_enable == 1"
                                src="data:image/png;base64,"/>
                            <img 
                                class="w-5"
                                style="display:inline"
                                v-if="item.ea_enable == 0"
                                src="data:image/png;base64,"/>
                        </td>
                        <td class=" data ">
                            <span class="text-blue-450">
                                {{item.account}}
                            </span>  <br>
                            <span class="hidden-except-mobile" >
                                {{item.account_name}}
                            </span>
                        </td>
                        <td class=" data ">
                            {{item.profit}} 
                            <span class=" text-blue-450 hidden-except-mobile "> 
                                {{item.pnlday}}% 
                            </span>
                        </td>
                        <td class=" data ">{{item.equity}}</td>
                        <td class=" data ">{{item.floating}}</td>
                        <td class=" data hidden-mobile ">{{item.account_name}}</td>
                        <td class=" data hidden-mobile ">{{item.ea_enable}}</td>
                        <td class=" data hidden-mobile ">{{item.trade}}</td>
                        <td class=" data hidden-mobile ">{{item.broker_time}}</td>
                        <td class=" data hidden-mobile ">{{item.created_date}}</td>
                        <td class=" data hidden-mobile ">{{item.float_max}}</td>
                        <td class=" data hidden-mobile ">{{item.float_trade}}</td>
                        <td class=" data hidden-mobile ">{{item.float_date}}</td>
                        <td class=" data hidden-mobile ">{{item.day}}</td>
                        <td class=" data hidden-mobile ">{{item.rownum}}</td>
                        <td class=" data hidden-mobile ">{{item.dd}}</td>
                        <td class=" data hidden-mobile ">{{item.balance}}</td>
                        <td class=" data hidden-mobile ">{{item.pnlday}}</td>
                    </tr>
                </table>
            </div>
            <!-- <div class="block sm:block md:hidden lg:hidden" >
                <p class=" pl-2 py-4 text-3xl bg-white ">Last Data</p>
                <table 
                    class=" shadow-lg bg-white w-full ">
                    <tr 
                        class="w-full"
                        v-for="(item, index) in table" 
                        :key="index">
                        <td class="text-md flex flex-col w-full ">
                           <div class="flex flex-row ">
                                <div class="  label-custom-mobile "> Location </div>
                                <div class="w-full pl-2 py-1 "> {{item.location}} </div>
                            </div>
                            <div class="flex flex-row justify-between ">
                                <div class=" label-custom-mobile "> Account </div>
                                <div class="w-full pl-2 py-1 "> {{item.account_name}} </div>
                            </div>
                            <div class="flex flex-row w-full justify-between ">
                                <div class=" label-custom-mobile "> Profit </div>
                                <div class="w-full pl-2 py-1 "> 
                                    {{item.profit}} 
                                    <span class=" text-blue-450 ">
                                        ({{item.pnlday}}%)
                                    </span>
                                </div>
                            </div>
                            <div class="flex flex-row w-full justify-between ">
                                <div class=" label-custom-mobile "> Equity </div>
                                <div class="w-full pl-2 py-1 "> {{customEquity(item.equity)}} </div>
                            </div>
                            <div class="flex flex-row w-full justify-between ">
                                <div class=" label-custom-mobile "> Floating </div>
                                <div class="w-full pl-2 py-1 "> 
                                    {{item.floating}}  
                                    <span class=" text-blue-450 ">
                                        ({{item.dd}}%)
                                    </span>
                                </div>
                            </div>
                        </td>
                    </tr>
                </table>
            </div> -->
        </div>
    </main-layout>
</template>

<style lang="postcss" scoped>
    .label-column {
        @apply text-white text-center text-sm
        border px-3 py-1 bg-blue-450
    }
    .hidden-mobile {
        @apply md:block lg:block sm:hidden hidden
    }
    .hidden-except-mobile {
        @applly md:hidden lg:hidden sm:hidden block
    }
    .data {
        @apply border px-2 py-1 text-xs
    }
</style>

<script>
import axios from 'axios';
import MainLayout from '@/pages/MainLayout';

    export default {
        data() {
            return {
                token: localStorage.getItem('token'),
                table: {},
                loading: false,
                request: {
                    sorted_by: "trade_desc",
                },
            }
        },
        components: {
            MainLayout,
        },
         mounted() {
            this.getLastData();
        },
        methods: {
            async getLastData() {
                let that = this;
                this.loading = true;

                await axios.post(process.env.VUE_APP_BASE_URL + "api/dashboard/last-data",
                            this.request,
                                {headers: { Authorization: `Bearer `+ that.token}})
                            .then(responses => {
                                let status = responses.data.status;
                                let data = responses.data.data;

                                if(status){
                                    this.table = data;
                                    this.loading = false;
                                }
                            })
                            .catch(error => {
                                console.log(error);
                            });
            },
            customEquity(number) {
                return Intl.NumberFormat('de-DE', { style: 'currency', currency: 'USC' }).format(number);
            },
        },
    }
</script>

// version mobile
// location ea enable (hijau bundar)
// account account_name
// profit and pnlday %
// equity
// floating and dd %